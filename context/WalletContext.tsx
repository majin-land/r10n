...
export const WalletProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const retrieveWallet = async (): Promise<{
    walletAddress: string
    privateKey: string
    bib32RootKey: string
  } | null> => {
    try {
      ...
      if (storedAddress && storedPrivateKey && storedBib32RootKey) {
        ...
        return {
          ...
          bib32RootKey: storedBib32RootKey,
        }
      }
    } catch (error) {
  ...
  const clearWallet = async (): Promise<void> => {
    try {
      ...
      await AsyncStorage.removeItem(ACTIVITY_STEALTH_ADDRESS)
      await AsyncStorage.removeItem(USER_STEALTH_ADDRESS_COLLECTIONS)
