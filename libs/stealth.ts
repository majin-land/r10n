...
export async function generateStealthMetaAddress({
  ...
  const stealthMetaAddressWithoutPrefix =
    compressedSpendingPublicKey.slice(2) + compressedViewingPublicKey.slice(2)

  // Get the stealth meta address
  const stealthMetaAddress = 'st:base:0x' + stealthMetaAddressWithoutPrefix
